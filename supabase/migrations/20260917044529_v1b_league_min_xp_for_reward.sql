-- Per-tier minimum XP required to earn podium coins.
--
-- The previous migration gated coin payouts on xp_earned > 0, which stops the fully-idle
-- "won by default" case but still lets a single answer (3 XP) claim podium coins in a
-- low-activity division. This adds a tunable per-tier floor: a member only earns podium
-- coins if their frozen weekly XP clears leagues.min_xp_for_reward for their tier.
--
-- Gating is per member, per rank (independent) — a below-floor podium place earns nothing
-- while other qualifying places still pay. Default floor is 50 XP across all tiers; adjust
-- per tier in admin/SQL without a further migration. Achievement (podium/win) counts remain
-- on the xp_earned > 0 participation gate, unchanged.
--
-- Started from the live definition (pg_get_functiondef) per the migration ledger rules.

ALTER TABLE leagues
  ADD COLUMN IF NOT EXISTS min_xp_for_reward integer NOT NULL DEFAULT 50;

COMMENT ON COLUMN leagues.min_xp_for_reward IS
  'Minimum weekly XP a member must earn to qualify for podium coin rewards in close_league_week. Guards against "won by default" payouts in low-activity divisions. Gated per member, per rank.';
...
BEGIN
  ...
  -- Coin payout resolves by podium rank, but a member only qualifies if they cleared
  -- their tier's minimum weekly XP (leagues.min_xp_for_reward). Gated per member, per
  -- rank, so a below-floor podium place simply earns nothing while others still pay.
  UPDATE league_memberships lm
  SET coin_reward = lr.coin_reward
  FROM league_rewards lr, leagues l
  WHERE lm.week_start = v_week_start
    AND l.id = lm.league_id
    AND lr.league_id = lm.league_id
    AND lr.enabled
    AND lm.final_rank BETWEEN lr.rank_min AND lr.rank_max
    AND lm.xp_earned >= l.min_xp_for_reward;
