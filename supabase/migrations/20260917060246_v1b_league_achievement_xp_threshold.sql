-- Align placement-achievement counts with the per-tier coin threshold.
--
-- Coin payouts already require xp_earned >= leagues.min_xp_for_reward. This applies the
-- same qualifying bar to the podium/win achievement counts (league_first_podium,
-- league_first_win, league_wins_5), so a below-threshold finish no longer counts as a
-- podium or win. Historical finishes are re-evaluated against the tier's current
-- min_xp_for_reward via a join to leagues.
--
...
BEGIN
  ...
  -- Count only finishes that cleared the tier's coin threshold (leagues.min_xp_for_reward)
  -- — the same qualifying bar as coin rewards, so trophies track genuine podium finishes.
  FOR rec IN
    SELECT nm.user_id,
      (SELECT COUNT(*) FROM league_memberships h
         JOIN leagues hl ON hl.id = h.league_id
         WHERE h.user_id = nm.user_id AND h.final_rank BETWEEN 1 AND 3
           AND h.xp_earned >= hl.min_xp_for_reward) AS podiums,
      (SELECT COUNT(*) FROM league_memberships h
         JOIN leagues hl ON hl.id = h.league_id
         WHERE h.user_id = nm.user_id AND h.final_rank = 1
           AND h.xp_earned >= hl.min_xp_for_reward) AS wins
