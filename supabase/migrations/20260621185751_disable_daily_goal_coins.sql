-- Disable the daily-goal coin reward (keep the model)
--
-- Stops awarding the flat 20 coins for completing the daily XP goal. The coin
-- *model* stays fully intact (tables, award_coins, the daily_goal coin type +
-- its Target icon in Shop > History, the goal-completion notification) — only
-- the single grant call is removed from update_daily_activity step 5, so it can
-- be re-enabled later with a one-block migration. The daily goal remains a
-- self-motivation tool.
--
-- Full body copied verbatim from
-- 20260615133549_v1b_coin_amounts_in_notifications.sql:264-579; the ONLY change
-- is step 5's completion block (remove award_coins, drop the 'coins' template
-- key). Past coin_transactions rows of type daily_goal are untouched.
...
BEGIN
  ...
  --
  -- Coin reward intentionally disabled: the daily goal still "completes"
  -- (daily_goal_met = true) and fires a coin-free completion notification,
  -- but no longer grants coins. To re-enable, re-add the award_coins call
  -- below and restore the 'coins' key on the goal.daily_complete template.
  -- ------------------------------------------------------------------------
...
-- Revert the notification template copy so it no longer advertises coins.
UPDATE public.notification_templates
SET message       = 'You hit your daily XP goal ({xp}/{goal}). Nice work!',
    toast_message = '{xp}/{goal} XP — daily goal complete!',
    updated_at    = now()
